...
gegl_node_dispose (GObject *gobject)
{
  ...
  if (self->vips_image)
    {
      g_object_unref (self->vips_image);
      self->vips_image = NULL;
    }
...
gegl_node_process (GeglNode *self)
{
  ...
  printf( "gegl_node_process:\n" );

  /* Walk the graph bottom-up calling all the _prepare() methods. This will
   * trigger the vips calls.
   */

  id = g_malloc (1);
  g_object_ref (self);
  prepare_visitor = g_object_new (GEGL_TYPE_PREPARE_VISITOR, "id", id, NULL);
  gegl_visitor_dfs_traverse (prepare_visitor, GEGL_VISITABLE (self));
  g_object_unref (prepare_visitor);
  g_object_unref (self);
  g_free (id);
}
...
void
gegl_node_dump (GeglNode * node, int indent)
{
  int n;
  GeglNode *input;

  spc (indent);
  printf ("node %s (%p)", G_OBJECT_TYPE_NAME (node), node);
  if (node->vips_image)
    printf (", vips_image %s (%p)",
	    node->vips_image->filename, node->vips_image);
  if (node->operation)
    {
      input = gegl_operation_get_source_node (node->operation, "input");
      if (input)
	printf (", input %p", input);
    }
  printf ("\n");

  n = gegl_node_get_num_children (node);
  if (n > 0)
    {
      int i;

      for (i = 0; i < n; i++)
	gegl_node_dump (gegl_node_get_nth_child (node, i), indent + 2);
    }
}

void
gegl_vips_error (const char *domain)
{
  g_warning ("%s: vips error %s\n", domain, vips_error_buffer ());
  vips_error_clear ();
}
